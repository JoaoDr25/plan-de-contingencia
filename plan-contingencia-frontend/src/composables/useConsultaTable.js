...
export function usePlanesConsultaTable({ sourceRows, defaultRowsPerPage = 8 }) {
  ...
  const filteredRows = computed(() => {
    ...
    return rows.filter((row) => {
      ...
      const matchesCodigo =
        !codigo || normalizeText(row.codigoPlan) === codigo || normalizeText(row.numero) === codigo

      const matchesFicha =
        !ficha.value || normalizeText(row.ficha).includes(normalizeText(ficha.value))

      const matchesPrograma =
        !programaFormacion.value ||
        normalizeText(row.programaFormacionNombre).includes(normalizeText(programaFormacion.value))

      const matchesActividad =
        !actividad.value ||
        normalizeText(row.descripcionActividad).includes(normalizeText(actividad.value))

      const matchesInstructor =
        !instructor.value ||
        normalizeText(row.instructorNombre).includes(normalizeText(instructor.value))
  ...
  const endRow = computed(() => {
    ...
    return Math.min(currentPage.value * rowsPerPage.value, filteredRows.value.length)
  })
  ...
  watch(
    [codigoPlan, ficha, programaFormacion, actividad, instructor, selectedStatus, dateFrom, dateTo],
    () => {
      currentPage.value = 1
    },
  )
