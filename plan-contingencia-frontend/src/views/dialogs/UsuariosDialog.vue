<template>
  <BaseDialog v-model="dialog" title="Actualizar Usuario" width="700px">
    <BaseFormGrid :columns="2">
      ...
      <div class="firma-field">
        <q-file
          ...
          :display-value="firmaFile?.name || form.firmaNombre || ''"
          max-file-size="1048576"
          @rejected="handleFirmaRejected"
        >
          <template #prepend>
            <q-icon name="draw" />
          </template>
    ...
    <template #actions>
      <BaseDialogActions save-label="Actualizar" @save="handleSave" @cancel="closeDialog" />
    </template>
  </BaseDialog>
</template>
...
const { modelValue, user, canEditUserConfiguration } = defineProps({
  modelValue: {
    type: Boolean,
    required: true,
  },
...
function validateForm() {
  const fieldsToValidate = USER_FORM_FIELDS.filter((field) =>
...
function handleFirmaRejected(rejectedEntries) {
  ...
  firmaError.value = 'La imagen debe ser PNG, JPG o WEBP y no superar 1 MB.'
}
