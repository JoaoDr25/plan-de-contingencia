  ...
  <section class="plan-security">
    <div class="plan-security__grid">
      <section class="security-section">
        ...
        <div class="epp-list">
          <div v-for="epp in eppOptions" :key="epp._id" class="epp-item">
            <q-checkbox
              :model-value="isEppSelected(epp._id)"
              size="sm"
              dense
              @update:model-value="(value) => handleEppSelection(epp._id, value)"
            />
        ...
        <div class="security-notice">
          ...
          <span>
            Asegúrese de que los aprendices lleven los EPP seleccionados y se encuentren en buen
            estado.
          </span>
        </div>
      </section>
        ...
        <div class="emergency-contacts-card">
          <div class="emergency-contacts-scroll">
            <div class="emergency-contacts-content">
              ...
              <div v-if="selectedContacts.length" class="selected-contacts">
                <div
      ...
      <section v-if="plan.seguridadVial.aplica" class="security-section security-section--vial">
        <div class="section-title">
        ...
        >
          <template #body="{ rows, gridStyle }">
            <div v-for="row in rows" :key="row.itemId" class="security-vial-row" :style="gridStyle">
              ...
              <div class="security-vial-status">
                <q-radio
                  ...
                  label="Cumple"
                  dense
                  @update:model-value="(value) => handleSecurityStatusChange(row.itemId, value)"
                />
...
function initializeSecurityVialItems() {
  ...
  plan.seguridadVial.aplica = plan.tipoTransporte !== 'APRENDIZ'
...
function handleSecurityStatusChange(itemId, value) {
  const item = plan.seguridadVial.items.find((securityItem) => securityItem.itemId === itemId)
...
function removeContact(id) {
  ...
  const index = plan.contactosEmergencia.contactosBase.indexOf(id)
...
function validate() {
  ...
  const securityStatusValid = plan.seguridadVial.items.every((item) => item.cumple !== null)
