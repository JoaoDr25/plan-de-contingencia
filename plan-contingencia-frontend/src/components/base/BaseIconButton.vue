...
const { action, size, disabled } = defineProps({
  action: {
    type: String,
    required: true,
  },
...
const config = computed(() => {
  return CRUD_ACTIONS[action] || CRUD_ACTIONS.view
})

const button = computed(() => {
  return ICON_BUTTON_VARIANTS[config.value.variant] || ICON_BUTTON_VARIANTS.success
})
</script>
