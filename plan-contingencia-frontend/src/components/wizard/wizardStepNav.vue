<template>
  <nav class="wizard-step-nav" aria-label="Pasos del plan de contingencia">
    <button
      ...
      :disabled="isLockedStep(step.number)"
      :aria-current="isCurrentStep(step.number) ? 'step' : undefined"
      @click="handleStepClick(step.number)"
    >
      <span
        class="wizard-step-nav__indicator"
        :class="{
      ...
      >
        <q-icon
          v-if="isCompletedStep(step.number) && !isCurrentStep(step.number)"
          name="check"
          size="13px"
        />
...
function isLockedStep(stepNumber) {
  return stepNumber !== props.currentStep && !isCompletedStep(stepNumber)
}
