const LOCAL_USER_FIELDS = ['rol', 'estado', 'firma', 'firmaNombre']
...
export function mergeUsersFromRepfora(localUsers, repforaUsers) {
  ...
  const mergedUsers = repforaUsers.map((repforaUser) => {
    ...
    if (!localUser) {
      ...
      return {
        ...repforaUser,
        rol: String(repforaUser.rol || 'CONSULTOR')
          .trim()
          .toUpperCase(),
  ...
  const repforaDocuments = new Set(repforaUsers.map((user) => normalizeDocument(user.documento)))
