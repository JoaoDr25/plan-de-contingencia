...
export function isPlanOwner(plan, currentUser) {
  ...
  if (
    planOwnerId &&
    currentUserId &&
    normalizeValue(planOwnerId) === normalizeValue(currentUserId)
  ) {
    return true
  }

  const currentUserName = [currentUser.nombre, currentUser.apellido].filter(Boolean).join(' ')
